...
`sensu_api_host`|String|Adress of the Sensu API server|`"127.0.0.1"`
`sensu_api_host`|String|Port of the Sensu API server|`4567`
`sensu_api_user`|String|User to connect to the api|`"sensu"`
`sensu_api_pass`|String|Password for the api user|`"placeholder"`
`sensu_api_auth`|String|auth component for api uri|`"{{ sensu_api_user}}:{{ sensu_api_pass }}"`
`sensu_api_sock`|String|socket component for api uri|`"{{ sensu_api_host }}:{{ sensu_api_port }}"`
`sensu_api_uri:`|String|combined api uri|`""http://{{ sensu_api_auth }}@{{ sensu_api_sock }}"`
